...
We therefore set out to work on merging the two data sets and filling the holes in the old data set with meaningful weather data from the second data set.

## More Problem with Data

When we are trying to bring the two data set together, we found that the relationships between the weather data of different areas are unsteady, which actually should have some equality because of the sharing of same weather stations. The result turn out to be that since not all the stations are at work in the same time, some councils have to even borrow weather data from other area, when the own weather station next to it is closed. This is a serious problem, we can now no more maintain that the weather data for each council is true.
...
names(db_merge)
```
Here are the columns present in the new merged data.
...
These 15 columns are not present in the first first set. We need to take values from the second set and include them in the first set. Here the problem is that the use of weather stations i inconsistent, as well as some of the stations are not present in all councils and some councils use weather data from a station which is unsuitable to represent the whole study area.
