...
#For both sexes  the rank of variables is (staring with the most relevant): weight, julianday, q_ media, ndvi.summer.1, ndvi.may.1.new, Perc.area.aperta
...
# ntree = 5000 ca 1 min computing time
# the same only male # ntree = 5000 ca 1 min computing time
RF_male3<-randomForest(horn ~  (q_media) + (q_min) + f.substrate + (density) + (ndvi.slop1) + (ndvi.maxincr1)  + (ndvi.summer1) + (ndvi.slop2) + (ndvi.maxincr2) + (Perc.area.aperta) + (snow_winter1) + (aspect) + (r_newsummer1) + (snow_winter2) + (r_spring2) + (r_newsummer2) + (twinter.mean1) + (tspring1.mean) + (tsummer1.mean) + (twinter.mean2) + (tspring2.mean) + (tsummer2.mean), ntree=5000, data=male_db)
varImpPlot(RF_male3)
#only female # ntree = 5000 ca 1 min computing time
RF_female3<-randomForest(horn ~  (q_media) + (q_min) + f.substrate + (density) + (ndvi.slop1) + (ndvi.maxincr1) + (ndvi.summer1) + (ndvi.slop2) + (ndvi.maxincr2) + (Perc.area.aperta) + (snow_winter1) + (aspect) + (r_newsummer1) + (snow_winter2) + (r_spring2) + (r_newsummer2) + (twinter.mean1) + (tspring1.mean) + (tsummer1.mean) + (twinter.mean2) + (tspring2.mean) + (tsummer2.mean), ntree=5000, data=female_db)
varImpPlot(RF_female3)
# both sexes
#omitting weight,Jday,x-council and y-council
#including both, ndvi.may1.new and ndvisummer1
RF5<-randomForest(horn ~ f.sex + (q_media) + (q_min) + f.substrate + (density) + (ndvi.slop1) + (ndvi.maxincr1) + (ndvi.may1.new)+ (ndvi.summer1) + (ndvi.slop2) + (ndvi.maxincr2) + (Perc.area.aperta) + (snow_winter1) + (aspect) + (r_newsummer1) + (snow_winter2) + (r_spring2) + (r_newsummer2) + (twinter.mean1) + (tspring1.mean) + (tsummer1.mean) + (twinter.mean2) + (tspring2.mean) + (tsummer2.mean), ntree=5000, data=db)
varImpPlot(RF5)


#Conclusion:
#Weight is the most relevant predictor followed by Julianday. The average elevation is predictor on rank 3 followed by ndvisummer1. Thereafter follow percentage of open area, ndvi.may1.new, sex and  minimum elevation (q_min).Density has about the same relevance than ndvimax.incr. Both, ndvi.maxincrement and ndvi.slope are more relevant predictors than temperature, rain or snow. Also substrate and aspect play a minor role. The relevance of the selected variables for the horngrowth is about the same in both sexes.
